...
function handValue(hand)
{
    ...
    for (const card of hand) {
        if (card == 'J') jCount++;
        else cardCount.set(card, (cardCount.get(card) ?? 0) + 1);
    }
    if (jCount == 5) return 6;
    const countArray = Array.from(cardCount.values()).sort().reverse();
    countArray[0] = countArray[0] + jCount;
    const countString = countArray.reverse().join("");
